...
class Audiofile:
    ...
    def splice_file(self, other, time):
        ...
        if isinstance(other, Audiofile):
            ...
            self.stHeaderFields['Subchunk2Size'] = (self.stHeaderFields['Subchunk2Size']
                                                    + other.stHeaderFields['Subchunk2Size'])
            self.stHeaderFields['ChunkSize'] = self.stHeaderFields['Subchunk2Size'] + 36
            self.header = self.header.replace(self.header[40:44],
                                              struct.pack('<L', self.stHeaderFields['Subchunk2Size']))
            self.header = self.header.replace(self.header[4:8],
                                              struct.pack('<L', self.stHeaderFields['ChunkSize']))
            self.stHeaderFields['SizeSec'] = self.stHeaderFields['SizeSec'] + other.stHeaderFields['SizeSec']
            self.output_in_file()
        else:
    ...
    def speed_multiplying(self, speed):
        ...
        self.stHeaderFields['SampleRate'] = int(self.stHeaderFields['SampleRate']*speed)
        self.stHeaderFields['Subchunk2Size'] = int(self.stHeaderFields['Subchunk2Size'] * 1/speed)
        self.stHeaderFields['ChunkSize'] = self.stHeaderFields['Subchunk2Size'] + 36
        self.header = self.header.replace(self.header[40:44], struct.pack('<L', self.stHeaderFields['Subchunk2Size']))
        self.header = self.header.replace(self.header[4:8], struct.pack('<L', self.stHeaderFields['ChunkSize']))
        self.header = self.header.replace(self.header[24:28], struct.pack('<L', self.stHeaderFields['SampleRate']))
        self.stHeaderFields['SizeSec'] = self.stHeaderFields['SizeSec'] * 1/speed
        self.output_in_file()
